...
export const dashboardTabs = ["My plan", "Exams", "Skills", "Career", "Milestones", "Notes"] as const;
...
const tabDetails: Record<DashboardTab, { title: string; subtitle: string; items: string[] }> = {
  "My plan": {
    ...
    items: ["Review your target exam options", "Practise one timed problem set", "Write down questions for your mentor"],
  },
  Exams: {
    title: "Make exam preparation feel manageable",
    subtitle: "Keep the syllabus, practice and milestones in view.",
    items: ["Choose the next topic to revise", "Try a short timed practice set", "Review what felt difficult"],
  },
  Skills: {
    title: "Build skills with useful practice",
    subtitle: "Turn a broad interest into a small project or learning goal.",
    items: ["Pick one skill to strengthen", "Set aside time for deliberate practice", "Save one example of your work"],
  },
  Career: {
    title: "Explore before you commit",
    subtitle: "Compare options with your interests and current strengths.",
    items: ["List roles you want to understand", "Find the skills each path asks for", "Choose one person to learn from"],
  },
  Milestones: {
    title: "Notice the progress you are making",
    subtitle: "Break long-term goals into steps you can revisit.",
    items: ["Review a completed learning goal", "Update your short-term priority", "Set a realistic next milestone"],
  },
  Notes: {
    title: "Keep the questions that matter close",
    subtitle: "Bring your reflections and decisions into one clear view.",
    items: ["Write down what is still unclear", "Capture advice you want to revisit", "Prepare for your next check-in"],
  },
};
      ...
      <aside className="iv-dash-sidebar">
        ...
        <div className="iv-dash-tabs" role="tablist" aria-label="Explore the student plan preview">
          {dashboardTabs.map((name, index) => (
            ...
            >
              <Icon name={tabIcons[index]} size={15} />{name}
...
export function MiniWorkflow() {
  const items: { icon: IconName; title: string; text: string }[] = [
    ...
    { icon: "courses", title: "Make a practical plan", text: "Turn choices into manageable steps." },
    { icon: "assignments", title: "Learn by doing", text: "Build confidence through practice." },
