...
export function MotionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const scrollTo = (target: HTMLElement) => {
      ...
      const duration = Math.min(1000, Math.max(650, Math.abs(distance) * .12));
  ...
  return <MotionConfig reducedMotion="user" transition={{ duration: .55, ease: [.22, 1, .36, 1] }}>{children}</MotionConfig>;
}
